...
Fit a GAM using a tensor product of the RW MRF basis and the phylogenetic MRF basis. Set `drop.unused.levels = FALSE` to ensure there are no errors because of the extra species and times in the penalty matrices
```{r}
mod <- gam(y ~ te(time_factor, species, 
                  bs = c("mrf", "mrf"),
                  k = c(8, N_species),
...
Hot Damn it worked! But could we recover these missing trends without the information provided in the phylogenetic structure? Fit a second GAM that uses a hierarchical smooth of time
```{r}
